...
use quinn_proto::{Connection, Event, StreamEvent, StreamId, WriteError};
...
impl AsyncWrite for SendStream {
    ...
    fn poll_write_vectored(
        ...
        if slots - n <= QUIC_STREAM_SEND_UNBLOCK_THRESHOLD
        ...
        loop {
            ...
            writable_streams.retain(|id| {
                ...
                loop {
                    ...
                    let mut chunks = ctx.send.read_chunk(slots).unwrap();
                    let slice = chunks.as_mut_slices().0;
                    let expect = slice.len();
                    let written = stream.write_chunks(slice);
                    match written {
                        Ok(written) => {
                            let written = written.chunks;
                            chunks.commit(written);
