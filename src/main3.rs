...
async fn benchmark_throughput() {
    ...
    let c2s = tokio::spawn(async move {
        ...
        while let Some(pkt) = rx.recv().await {
            ...
            s_arc.lock().await.send(CLIENT_ADDR.parse().unwrap(), pkt.payload).unwrap();
        }
    });
    ...
    let s2c = tokio::spawn(async move {
        ...
        while let Some(pkt) = rx.recv().await {
            ...
            c_arc.lock().await.send(SERVER_ADDR.parse().unwrap(), pkt.payload).unwrap();
        }
    });
    ...
    let client_stream_task = tokio::spawn(async move {
        ...
        let mut stream = loop {
            ...
            {
                trace!("Client: 尝试打开流...");
                let mut endpoint = client.lock().await;
                // 尝试打开流
                if let Ok(s) = endpoint.open(server_addr, None) {
        ...
        // 拿到流之后继续业务逻辑...
        let payload_size = 1024 * 1024 * 4096; // 1GB
...
async fn benchmark_latency_pps() {
    ...
    // Client: Ping Pong
    let mut endpoint = client.lock().await;
    let mut stream = endpoint.open(SERVER_ADDR.parse().unwrap(), None).unwrap();
    drop(endpoint);
